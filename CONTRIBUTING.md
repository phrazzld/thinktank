...
4.  **Code Quality:** Use formatters (`npm run format`) and linters (`npm run lint`). Do not disable linting. Use clear, self-descriptive names. Prioritize readability. Ensure files end with a newline (`npm run fix:newlines`). Follow `.editorconfig`.
...
* **Execution:** Run tests via `npm test`. Ensure all tests pass before committing.
...
3.  **Lint & Format:** Run `npm run lint` and `npm run format`. Fix all issues.

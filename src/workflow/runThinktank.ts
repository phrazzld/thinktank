 ...
 */
import { styleHeader, styleDim, styleWarning } from '../utils/consoleUtils';
...
export async function runThinktank(
  ...
  try {
    ...
    const modelList = modelSelectionResult.models
      .map(
        (model, index) =>
          `  ${index + 1}. ${model.provider}:${model.modelId}${!model.enabled ? ' (disabled)' : ''}`
      )
      .join('\n');
    ...
    const summaryData: CompletionSummaryData = {
      ...
      successCount: Object.values(queryResults.queryResults.statuses).filter(
        s => s.status === 'success'
      ).length,
      failureCount: Object.values(queryResults.queryResults.statuses).filter(
        s => s.status === 'error'
      ).length,
      errors: Object.entries(queryResults.queryResults.statuses)
        ...
        .map(([modelKey, status]) => ({
          ...
          category:
            status.detailedError && 'category' in status.detailedError
      ...
      totalExecutionTimeMs:
        queryResults.queryResults.timing.durationMs + fileOutputResult.timing.durationMs,
    };
    ...
    if (options.includeMetadata) {
      ...
      consoleLogger.plain(
        styleDim(`  Total API calls:    ${queryResults.queryResults.timing.durationMs}ms`)
      );
      consoleLogger.plain(
        styleDim(`  File writing:       ${fileOutputResult.timing.durationMs}ms`)
      );

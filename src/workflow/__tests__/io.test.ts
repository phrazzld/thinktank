...
import { setupBasicFs, createMockFileSystem } from '../../../test/setup/fs';
...
describe('I/O Module', () => {
  describe('createDirectory', () => {
    ...
    it('should throw FileSystemError when throwOnError is true', async () => {
      ...
      // Act & Assert
      await expect(createDirectory(testDirPath, fileSystem, options)).rejects.toThrow(
        FileSystemError
      );
      ...
      await expect(createDirectory(testDirPath, fileSystem, options)).rejects.toThrow(
        `Failed to create directory: ${testDirPath}`
      );
    });
  });
  ...
  describe('writeFiles', () => {
    ...
    it('should write all files successfully', async () => {
      ...
      // Check file contents
      expect(await fileSystem.readFileContent(path.join(outputDir, 'file1.md'))).toBe(
        '# Content for file 1'
      );
      expect(await fileSystem.readFileContent(path.join(outputDir, 'file2.md'))).toBe(
        '# Content for file 2'
      );
    });
    ...
    it('should throw error when output directory creation fails and throwOnError is true', async () => {
      ...
      // Act & Assert
      await expect(writeFiles(files, outputDir, fileSystem, options)).rejects.toThrow(
        FileSystemError
      );
    ...
    it('should throw error on file write failure when throwOnError is true', async () => {
      ...
      await expect(writeFiles(files, outputDir, fileSystem, options)).rejects.toThrow(
        `Failed to write file: ${path.join(outputDir, 'file2.md')}`
      );
    });
    ...
    it('should create parent directories for each file if needed', async () => {
      ...
      expect(await fileSystem.readFileContent(nestedFilePath)).toBe('# Content for nested file');
    });

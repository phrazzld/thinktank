...
jest.mock('../runThinktank', () => {
  ...
  return {
    ...
    runThinktank: jest.fn().mockImplementation(async (options: RunOptions) => {
      ...
      await (outputHandler.writeResponsesToFiles as jest.Mock)(responses, outputDirPath, {
...
describe('Output Directory Feature', () => {
  ...
  // Use normalizePathGeneral for these paths as they're not directly used with memfs
  const mockFullOutputDir = normalizePathGeneral(path.join(mockOutputDir, mockRunDirectoryName));
      ...
      files: [
        {
          ...
          filePath: normalizePathGeneral(path.join(mockFullOutputDir, 'provider-a-model-1.md')),
          status: 'success',
          startTime: Date.now(),
          endTime: Date.now(),
          durationMs: 0,

...
describe('OutputHandler', () => {
  ...
  describe('Directory Creation', () => {
    ...
    it('should throw error if directory creation fails', async () => {
      ...
      await expect(createOutputDirectory({}, mockFileSystem)).rejects.toThrow(
  ...
  describe('File Data Preparation (Pure)', () => {
    ...
    it('should respect includeMetadata option', () => {
      ...
      const fileData1 = writeResponsesToFiles([sampleResponseWithMetadata], '/test/output/dir', {
        includeMetadata: true,
      });
      ...
      const fileData2 = writeResponsesToFiles([sampleResponseWithMetadata], '/test/output/dir', {
    ...
    it('should include error information when present', () => {
      ...
      const fileData = writeResponsesToFiles([sampleResponseWithError], '/test/output/dir');
  ...
  describe('File Writing (Impure)', () => {
    it('should write file data to disk', async () => {
      ...
      const result = await writeFilesToDisk(fileData, '/test/output/dir', {}, mockFileSystem);
      ...
      expect(
        virtualFs.existsSync('/test/output/dir/coding-anthropic-claude-3-opus-20240229.md')
      ).toBe(true);
    ...
    it('should handle throwOnError option', async () => {
      ...
      // Prepare file data using the pure function
      const fileData = writeResponsesToFiles([sampleResponse], '/test/output/dir');
      ...
      mockFileSystem.writeFile.mockRejectedValueOnce(
        createFsError('ENOSPC', 'Write failed', 'open', '/test/output/dir/file.md')
      );
  ...
  describe('Pure Output Processing', () => {
    it('should process output for both files and console without IO operations', () => {
      const result = processOutput([sampleResponse, sampleResponseWithGroup], {
        includeMetadata: true,
        useTable: true,
      });

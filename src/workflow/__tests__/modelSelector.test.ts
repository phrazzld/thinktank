...
describe('Model Selector', () => {
  ...
  beforeEach(() => {
    ...
    const mockGetEnabledModels = configManager.getEnabledModels as jest.MockedFunction<
      typeof configManager.getEnabledModels
    >;
    ...
    const mockGetEnabledModelsFromGroups =
      configManager.getEnabledModelsFromGroups as jest.MockedFunction<
        typeof configManager.getEnabledModelsFromGroups
      >;
    ...
    const mockFindModel = configManager.findModel as jest.MockedFunction<
      typeof configManager.findModel
    >;
    ...
    const mockGetProviderIds = llmRegistry.getProviderIds as jest.MockedFunction<
      typeof llmRegistry.getProviderIds
    >;
  ...
  describe('Multiple Model Selection', () => {
    ...
    it('should handle errors in multiple model selection', () => {
      ...
      expect(
        result.warnings.some(warning => warning.includes('Model "unknown:model" not found'))
      ).toBe(true);
    ...
    it('should throw when all models are invalid and throwOnError is true', () => {
      ...
      try {
        ...
        expect(
          typedError.suggestions?.some(suggestion =>
  ...
  describe('Group-based Selection', () => {
    ...
    it('should handle group errors when throwOnError is false', () => {
      ...
      expect(result.warnings.length).toBeGreaterThan(0);
      expect(
        result.warnings.some(warning => warning.includes('Group "nonexistent" not found'))
      ).toBe(true);
    });
  ...
  describe('API Key Validation', () => {
    it('should validate API keys by default', () => {
      ...
      expect(result.warnings.length).toBeGreaterThan(0);
      expect(result.warnings.some(warning => warning.includes('Missing API keys'))).toBe(true);
    });
    ...
    it('should provide the correct message when no models are found', () => {
      ...
      try {
        ...
        expect((error as Error).message).toContain(
          'No enabled models found in the specified group'
        );
        expect((error as ModelSelectionError).category).toBe('Configuration');
      }

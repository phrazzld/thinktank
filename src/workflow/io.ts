...
export function logFileOutputResult(
  ...
  // Log summary
  if (failedWrites === 0) {
    logger.success(`${styleSuccess('✓')} Wrote ${succeededWrites} ${succeededWrites === 1 ? 'file' : 'files'} to ${outputDirectory}`);
  } else if (succeededWrites > 0) {
    logger.warn(`${styleWarning('⚠')} Wrote ${succeededWrites} ${succeededWrites === 1 ? 'file' : 'files'} to ${outputDirectory} (${failedWrites} failed)`);
  } else {

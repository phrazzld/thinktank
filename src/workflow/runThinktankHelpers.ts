...
export async function _setupWorkflow({
  ...
  try {
    ...
    const outputDirectoryPath = await createOutputDirectory(
      ...
      fileSystem
    ); // Use the shared fileSystem instance
    ...
    if (
      ...
      throw new FileSystemError(`File system error: ${error.message}`, {
        cause: error,
        suggestions: ['Check disk space and permissions', 'Verify the path is valid'],
      });
    }
    ...
    throw new ConfigError(
      `Error during workflow setup: ${error instanceof Error ? error.message : String(error)}`,
      {
        cause: error instanceof Error ? error : undefined,
      }
    );
...
export async function _processInput({
  ...
  try {
    ...
    try {
      ...
      throw new FileSystemError(
        `Failed to read context files: ${err instanceof Error ? err.message : String(err)}`,
        {
    ...
    const successfulContextFiles = contextFiles.filter(
      file => file.error === null && file.content !== null
    );

    // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    const errorContextFiles = contextFiles.filter(
      file => file.error !== null || file.content === null
    );
    ...
    if (errorContextFiles.length > 0) {
      spinner.warn(
        styleWarning(
          `${errorContextFiles.length} of ${contextFiles.length} context files could not be read and will be skipped.`
        )
      );
    ...
    if (successfulContextFiles.length > 0) {
      ...
      spinner.info(
        styleInfo(
          `Added ${successfulContextFiles.length} context file${successfulContextFiles.length === 1 ? '' : 's'} to the prompt.`
        )
      );
      ...
      spinner.warn(
        styleWarning('No context files could be read. Continuing with original prompt only.')
      );
    ...
    if (
      ...
      if (nodeError.code === 'EACCES' || nodeError.code === 'EPERM') {
        throw new FileSystemError(`Permission denied when accessing input: ${error.message}`, {
          cause: error,
          filePath: input,
          suggestions: ['Check file permissions', 'Try using a different input file'],
        });
      }
    ...
    throw new FileSystemError(`File not found: ${input}`, {
      filePath: input,
      cause: error instanceof Error ? error : undefined,
      suggestions: ['This is an unexpected error', 'Try a different input method or file'],
    });
  }
}
...
export function _selectModels({
  ...
  try {
    ...
    spinner.info(
      styleInfo(
        `Using ${modelCount} ${modelCount === 1 ? 'model' : 'models'}: ${styleSuccess(modelList)}`
      )
    );
    spinner.start(); // Restart spinner for next step
    ...
    throw new ConfigError(
      `Error selecting models: ${error instanceof Error ? error.message : String(error)}`,
      {
        cause: error instanceof Error ? error : undefined,
        suggestions: [
...
export async function _executeQueries({
  ...
  try {
    ...
    const queryPromises: Array<
      Promise<import('../core/types').LLMResponse & { configKey: string }>
    > = [];
    ...
    for (const model of models) {
      ...
      const queryPromise = (async () => {
        ...
        try {
          ...
          const category = error instanceof ThinktankError ? error.category : errorCategories.API;
    ...
    if (failureCount === 0) {
      ...
      spinner.info(
        styleInfo(
          `Queried ${successCount} ${successCount === 1 ? 'model' : 'models'} in ${totalTime.toFixed(2)}s`
        )
      );
      ...
      spinner.warn(
        styleWarning(`Some models failed to respond (${failureCount} of ${models.length})`)
      );
      ...
      spinner.warn(
        styleWarning(
          `All models failed to respond (${failureCount} ${failureCount === 1 ? 'model' : 'models'})`
        )
      );

      // Display failure details
      const errorSummary = responses.map(r => `${r.configKey}: ${r.error}`).join(', ');
...
export function _processOutput({
  ...
  try {
    ...
    throw new ThinktankError(
      `Error formatting output: ${error instanceof Error ? error.message : String(error)}`,
      {
        cause: error instanceof Error ? error : undefined,
...
export function _handleWorkflowError({
  ...
  consoleLogger?: import('../core/interfaces').ConsoleLogger;
}): never {

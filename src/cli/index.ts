...
function addCategorySpecificGuidance(error: ThinktankError): void {
    ...
    if (error instanceof ApiError && error.providerId) {
      ...
      switch (error.providerId.toLowerCase()) {
        ...
        case 'google':
          logger.error('\nGoogle AI API help:');
          logger.error(
            `  ${colors.green('>')} Get API keys: https://aistudio.google.com/app/apikey`
          );
        ...
        default:
          logger.error('\nAPI help:');
          logger.error(
            `  ${colors.green('>')} Ensure you have the correct API key for ${error.providerId}`
          );
          logger.error(
            `  ${colors.green('>')} Set with: export ${error.providerId.toUpperCase()}_API_KEY=your_key_here`
          );
      }
    } else {
  ...
  // Network errors
    ...
    logger.error(
      `  ${colors.green('>')} Verify you can access the API endpoints (no firewall blocking)`
    );
  ...
  // Validation errors (including input errors)
    ...
    error.category === errorCategories.INPUT
  ) {
    logger.error('\nInput help:');
  ...
  // For unknown or other errors, offer general debugging help
    ...
    logger.error(
      `  ${colors.green('>')} Report bugs at: https://github.com/phrazzld/thinktank/issues`
    );
  }
}

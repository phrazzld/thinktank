...
describe('Config Command Basic Tests', () => {
  ...
  beforeEach(() => {
    ...
    (configManager.getActiveConfigPath as jest.Mock).mockResolvedValue(
      '/mock/path/thinktank.config.json'
    );
  ...
  it('should handle errors through handleError function', async () => {
    ...
    (configManager.getActiveConfigPath as jest.Mock).mockRejectedValueOnce(
      new Error('Test config error')
    );

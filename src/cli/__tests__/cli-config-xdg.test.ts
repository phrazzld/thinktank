...
const getActiveConfigPath = configManager.getActiveConfigPath as jest.MockedFunction<
  typeof configManager.getActiveConfigPath
>;
...
describe('CLI Config XDG Integration Tests', () => {
  ...
  describe('config commands with paths', () => {
    it('should include a config option for the show command', () => {
      ...
      const configOption = showCommand?.options.find(
    ...
    it('should include a config option for all model commands', () => {
      ...
      modelsCommand?.commands.forEach(cmd => {
        const configOption = cmd.options.find(
          opt => opt.flags.includes('--config') || opt.flags.includes('-c')
        );

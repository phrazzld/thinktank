...
// Access the mocks
const runThinktank = runThinktankModule.runThinktank as jest.MockedFunction<
  typeof runThinktankModule.runThinktank
>;
const getConfigFilePath = fileReader.getConfigFilePath as jest.MockedFunction<
  typeof fileReader.getConfigFilePath
>;
...
describe('Run Command with XDG Configuration Integration', () => {
  ...
  describe('run command with default XDG config', () => {
    it('should have a structure to load config from XDG path by default', async () => {
      ...
      const configOption = runCommand.options.find(
        opt => opt.flags.includes('--config') || opt.flags.includes('-c')
      );
    ...
    it('should have a structure to support group options', async () => {
      ...
      const groupOption = runCommand.options.find(
        opt => opt.flags.includes('--group') || opt.flags.includes('-g')
      );

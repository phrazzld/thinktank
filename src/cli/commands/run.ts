...
runCommand
  ...
  .argument(
    '[contextPaths...]',
    'Optional paths to files or directories to include as context. Can be individual files, directories (which will be recursively traversed), or a mix of both. Directories respect .gitignore patterns.'
  )
  .option(
    '-m, --models <models>',
    'Comma-separated list of specific models to use (e.g., openai:gpt-4o,anthropic:claude-3-opus)'
  )
  ...
  .option(
    '--disable-spinner-throttling',
    'Disable spinner update throttling (may cause more terminal flicker)'
  )
  .action(
    async (
      promptFile: string,
      ...
      try {
        ...
        try {
          ...
          const fileError = createFileNotFoundError(
            promptFile,
            `Input file not found: ${promptFile}`
          );
        ...
        if (options.verbose && contextPaths.length > 0) {
          logger.info(
            colors.cyan(
              `Including ${contextPaths.length} context path${contextPaths.length === 1 ? '' : 's'}`
            )
          );
        }
        ...
        if (options.models) {
          ...
          logger.info(
            colors.cyan(
              `Running prompt against ${specificModels.length} model${specificModels.length === 1 ? '' : 's'}:`
            )
          );
        ...
        if (options.group) {
          const configOptions: configManager.LoadConfigOptions = options.config
          ...
          if (!groupExists) {
            ...
            const availableGroups = configWithGroups.groups
              ? Object.keys(configWithGroups.groups)
              : [];
            ...
            throw new ConfigError(`Model group "${options.group}" not found in config`, {
              suggestions: [
                ...
                'Available groups: ' +
                  (availableGroups.length > 0 ? availableGroups.join(', ') : 'none defined'),
          ...
          if (options.verbose) {
            ...
            logger.info(
              colors.cyan(
                `Running with model group "${options.group}" (${groupModels.length} models)`
              )
            );
            if (groupModels.length > 0) {
              groupModels.forEach((model, index) => {
                logger.info(
                  `  ${index + 1}. ${colors.yellow(`${model.provider}:${model.modelId}`)}`
                );
              });
              logger.info('');
            } else {
              logger.warn(colors.yellow('Warning: Selected group has no enabled models'));
            }
          }
        }
        ...
        if (!(error instanceof ThinktankError)) {
          handleError(
            new ThinktankError(error instanceof Error ? error.message : String(error), {
              cause: error instanceof Error ? error : undefined,
            })
          );
          return;
        ...
        if (error instanceof Error) {
          ...
          if (
            ...
            handleError(
              new ApiError(`API error: ${error.message}`, {
                cause: error,
                suggestions: [
          ...
          if (message.includes('config') || message.includes('configuration')) {
            handleError(
              new ConfigError(`Configuration error: ${error.message}`, {
                cause: error,
                suggestions: [

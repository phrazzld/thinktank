...
pathCommand.description('Show the path to the config file').action(async () => {
  try {
    ...
    if (await fileExists(configPath)) {
      ...
      // eslint-disable-next-line no-console
      console.log(
        `Status: ${colors.yellow('File does not exist yet')} (will be created when needed)`
      );
    }
    ...
    // eslint-disable-next-line no-console
    console.log(
      colors.dim('You can override this location with the --config option when running commands.')
    );
    ...
    if (projectLocalPath !== configPath) {
      ...
      console.log(
        colors.dim(
          'Project-local configs can be used to share settings with a team via version control.'
        )
      );
    }
  } catch (error) {
          ...
          if (config.models.length === 0) {
            ...
            Object.entries(modelsByProvider).forEach(([provider, models]) => {
              ...
              models.forEach(model => {
                const status = model.enabled ? colors.green('Enabled') : colors.red('Disabled');
          ...
          if (groupEntries.length === 0) {
            ...
            groupEntries.forEach(([groupName, group]) => {
              ...
              if (group.systemPrompt && group.systemPrompt.text) {
                const promptText =
                  group.systemPrompt.text.length > 50
  ...
  .action(
    async (options: {
      ...
      try {
        ...
        if (options.provider) {
          filteredModels = filteredModels.filter(model => model.provider === options.provider);
        }
      ...
      try {
        ...
        if (config.groups) {
          groupsContainingModel = Object.entries(config.groups)
            .filter(([_, group]) =>
              group.models.some(m => m.provider === provider && m.modelId === modelId)
            )
            .map(([name]) => name);
        }
        ...
        if (isInGroups && !options.force) {
          ...
          // eslint-disable-next-line no-console
          console.log(
            colors.yellow('Use --force to remove this model from all groups and the configuration')
          );
          return;
        }
  ...
  .action(async (identifier: string, options: { config?: string }) => {
    try {
      ...
      if (model.enabled) {
        // eslint-disable-next-line no-console
        console.log(
          colors.yellow(`Info: Model ${colors.cyan(`${provider}:${modelId}`)} is already enabled`)
        );
        return;
      }
  ...
  .action(async (identifier: string, options: { config?: string }) => {
    try {
      ...
      if (!model.enabled) {
        // eslint-disable-next-line no-console
        console.log(
          colors.yellow(`Info: Model ${colors.cyan(`${provider}:${modelId}`)} is already disabled`)
        );
        return;
      }
  ...
  .action(
    async (
      ...
      try {
        ...
        const modelsList: string[] = options.models ? options.models.split(',') : [];
        ...
        // Display success message
        // eslint-disable-next-line no-console
        console.log(colors.green(`Successfully created group "${colors.cyan(groupName)}"`));
        ...
        // eslint-disable-next-line no-console
        console.log(
          `  System Prompt: "${systemPrompt.text.substring(0, 50)}${systemPrompt.text.length > 50 ? '...' : ''}"`
        );
...
addModelToGroupCommand
  ...
  .action(async (groupName: string, modelIdentifier: string, options: { config?: string }) => {
    try {
      ...
      if (!config.groups || !config.groups[groupName]) {
        throw new loadConfig.ConfigError(`Group "${groupName}" not found in configuration`);
      }
      ...
      console.log(
        colors.green(
          `Successfully added model ${colors.cyan(`${provider}:${modelId}`)} to group "${colors.cyan(groupName)}"`
        )
      );
...
removeModelFromGroupCommand
  ...
  .action(async (groupName: string, modelIdentifier: string, options: { config?: string }) => {
    try {
      ...
      console.log(
        colors.green(
          `Successfully removed model ${colors.cyan(`${provider}:${modelId}`)} from group "${colors.cyan(groupName)}"`
        )
      );
  ...
  .action(
    async (
      ...
      try {
        ...
        // Display success message
        // eslint-disable-next-line no-console
        console.log(colors.green(`Successfully removed group "${colors.cyan(groupName)}"`));

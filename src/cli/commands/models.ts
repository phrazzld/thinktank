...
import { ConfigError, ValidationError, ApiError, NetworkError } from '../../core/errors';
...
modelsCommand
  ...
  .action(async (options: { provider?: string; config?: string }) => {
    try {
      ...
      if (error instanceof Error) {
        ...
        if (
          ...
          handleError(
            new ConfigError(`Invalid provider: ${options.provider}`, {
              suggestions: [

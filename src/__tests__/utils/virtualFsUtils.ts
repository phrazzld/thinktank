...
export function normalizePathForMemfs(inputPath: string): string {
  ...
  if (/^[A-Za-z]:[/\\]/.test(normalizedPath)) {
    ...
    normalizedPath =
      '/' + normalizedPath.charAt(0) + ':' + normalizedPath.slice(2).replace(/\\/g, '/');
  } else {
 ...
 * ```
 */
export async function addVirtualGitignoreFile(
  ...
  if (fileContents !== content) {
    throw new Error(
      `File content verification failed for ${normalizedPath}. Expected "${content}" but got "${String(fileContents)}"`
    );
  }
}

...
export function createModelFormatError(
  ...
  if (!modelSpecification.includes(':')) {
    suggestions.push(
      `Add a colon between provider and model ID: "${modelSpecification}" → "provider:${modelSpecification}"`
    );
  } else if (modelSpecification.endsWith(':')) {
    ...
    if (availableModels.length > 0) {
      ...
      if (matchingModels.length > 0) {
        const models =
          matchingModels.slice(0, 3).join(', ') + (matchingModels.length > 3 ? ', ...' : '');
    ...
    if (availableProviders.length > 0) {
      const providersList =
        availableProviders.slice(0, 5).join(', ') + (availableProviders.length > 5 ? ', ...' : '');
  ...
  if (availableModels.length > 0) {
    ...
    const exampleList = modelExamples.join(', ') + (availableModels.length > 5 ? ', ...' : '');
    suggestions.push(`Example models: ${exampleList}`);
  }

  // Add examples
  const examples = ['openai:gpt-4o', 'anthropic:claude-3-7-sonnet-20250219', 'google:gemini-pro'];
...
export function createModelNotFoundError(
  ...
  if (availableModels.length > 0) {
    ...
    if (sameProviderModels.length > 0) {
      const providerModelList =
        sameProviderModels.slice(0, 5).join(', ') + (sameProviderModels.length > 5 ? ', ...' : '');
    ...
    if (modelId) {
      ...
      if (similarModelIds.length > 0) {
        const similarList =
          similarModelIds.slice(0, 3).join(', ') + (similarModelIds.length > 3 ? ', ...' : '');
    ...
    const exampleList =
      availableModels.slice(0, 5).join(', ') + (availableModels.length > 5 ? ', ...' : '');
  ...
  const examples =
    availableModels.length > 0
      ...
      : ['openai:gpt-4o', 'anthropic:claude-3-7-sonnet-20250219', 'google:gemini-pro'];

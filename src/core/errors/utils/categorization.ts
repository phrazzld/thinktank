...
export const providerErrorCheckers = {
  isRateLimitError: (message: string): boolean =>
    testPatterns(message, providerErrorPatternMap.rateLimitPatterns),
  isTokenLimitError: (message: string): boolean =>
    testPatterns(message, providerErrorPatternMap.tokenLimitPatterns),
  isContentPolicyError: (message: string): boolean =>
    testPatterns(message, providerErrorPatternMap.contentPolicyPatterns),
  isAuthError: (message: string): boolean =>
    testPatterns(message, providerErrorPatternMap.authErrorPatterns),
  isNetworkError: (message: string): boolean =>
    testPatterns(message, providerErrorPatternMap.networkErrorPatterns),
};
 ...
 */
export function createContextualError(error: unknown, context: ErrorContext): ThinktankError {
  ...
  if (errorMessage.includes('model')) {
    if (errorMessage.includes('format') || errorMessage.includes('invalid')) {
      ...
      return new ConfigError(`Invalid model format: ${modelSpec}`, {
        ...
        examples: ['openai:gpt-4o', 'anthropic:claude-3-7-sonnet-20250219', 'google:gemini-pro'],
      });
  ...
  switch (category) {
    ...
    case errorCategories.CONFIG:
      thinktankError = new ConfigError(`Configuration error: ${errorObj.message}`, {
        cause: errorObj,
      });
      break;

    case errorCategories.FILESYSTEM:
      thinktankError = new FileSystemError(`File system error: ${errorObj.message}`, {
        cause: errorObj,
      });
      break;
    ...
    case errorCategories.VALIDATION:
      thinktankError = new ValidationError(`Validation error: ${errorObj.message}`, {
        cause: errorObj,
      });
      break;

...
export function filterModels(config: AppConfig, filter: string): ModelConfig[] {
  return config.models.filter(model => {
    ...
    return model.provider === filter || model.modelId === filter || combined === filter;
  });
}
...
export function findModelGroup(
  ...
  for (const [groupName, group] of Object.entries(config.groups)) {
    const isInGroup = group.models.some(
      groupModel => groupModel.provider === model.provider && groupModel.modelId === model.modelId
    );
  ...
  const isInDefaultModels = config.models.some(
    defaultModel =>
      defaultModel.provider === model.provider && defaultModel.modelId === model.modelId
  );
  ...
  try {
    ...
    try {
      ...
      throw new ConfigError(
        `Generated configuration JSON is invalid: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`
      );
    }
    ...
    if (error instanceof Error) {
      ...
      if ('code' in error) {
        ...
        if (error.code === 'EACCES') {
          throw new ConfigError(
            `Permission denied when saving configuration to ${configPath || 'default location'}. Check file permissions.`,
            error
          );
        }
        ...
        if (error.code === 'ENOSPC') {
          throw new ConfigError(
            `Not enough disk space to save configuration to ${configPath || 'default location'}.`,
            error
          );
        }
      }
...
export function findModel(
  ...
  return config.models.find(model => model.provider === provider && model.modelId === modelId);
}
 ...
 */
export function removeModel(config: AppConfig, provider: string, modelId: string): AppConfig {
  // Make a deep copy of the config
  const updatedConfig = structuredClone(config);
...
export function addOrUpdateGroup(
  ...
  try {
    ...
    const systemPrompt: SystemPrompt =
      groupDetails.systemPrompt || existingGroup?.systemPrompt || defaultSystemPrompt;

    // Use provided models, or existing, or empty array (in that order)
    const models: ModelConfig[] = groupDetails.models || existingGroup?.models || [];

    // Use provided description, or existing, or default (in that order)
    const description: string =
      groupDetails.description || existingGroup?.description || `Model group: ${groupName}`;
...
export function addModelToGroup(
  ...
  const modelExists = group.models.some(m => m.provider === provider && m.modelId === modelId);

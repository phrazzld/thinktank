...
describe('LLM Registry', () => {
  ...
  describe('registerProvider', () => {
    ...
    it('should throw an error when registering an undefined provider', () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect(() => registerProvider(undefined as any)).toThrow(
        'Cannot register undefined or null provider'
      );
    });
    ...
    it('should throw an error when registering a provider without providerId', () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      expect(() => registerProvider(invalidProvider as any)).toThrow(
    ...
    it('should throw an error when registering a duplicate provider', () => {
      ...
      expect(() => registerProvider(provider2)).toThrow(
        "Provider with ID 'test1' is already registered"
      );
    });
  });
  ...
  describe('callProvider', () => {
    ...
    beforeEach(() => {
      jest
        .spyOn(configManager, 'resolveModelOptions')
        .mockImplementation((_provider, _modelId, userOpts, groupOpts, cliOpts) => {
    ...
    it('should throw an error when the provider is not found', async () => {
      await expect(callProvider('nonexistent', 'model1', 'Test prompt')).rejects.toThrow(
        ProviderRegistryError
      );

      await expect(callProvider('nonexistent', 'model1', 'Test prompt')).rejects.toThrow(
        "Provider 'nonexistent' not found for model nonexistent:model1"
      );
    });
  });
});

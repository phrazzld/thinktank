...
import { colors, styleSuccess, styleError, styleDim, styleHeader } from './consoleUtils';
...
export function formatResponseForMarkdownFile(
  ...
  if (includeThinking && response.metadata?.thinking) {
    ...
    if (
      ...
      thinking !== null &&
      'process' in thinking &&
      typeof thinking.process === 'string'
    ) {
      lines.push('```text');
 ...
 */
export function formatResponse(response: LLMResponse, options: FormatOptions = {}): string {
 ...
 */
export function formatResponses(responses: LLMResponse[], options: FormatOptions = {}): string {
  ...
  const formattedResponses = responses.map(response => formatResponse(response, opts));
...
export function formatResultsTable(
  ...
  sortedResults.forEach(result => {
    ...
    const displayGroupName =
      groupName === 'default' ? (useColors ? styleDim('default') : 'default') : groupName;
    ...
    if (usage && typeof usage === 'object' && usage !== null) {
      ...
      if ('total_tokens' in usageRecord && typeof usageRecord.total_tokens === 'number') {
        ...
        'completion_tokens' in usageRecord &&
    ...
    const row: Cell[] = [result.configKey, displayGroupName, statusText, responseTime, tokens];
    table.push(row);
  ...
  if (includeMetadata) {
    ...
    groupedResults.forEach((groupResults, groupName) => {
      ...
      const groupTitle = groupName === 'default' ? 'Default Group' : `Group: ${groupName}`;

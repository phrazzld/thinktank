...
export class AnthropicProvider implements LLMProvider {
   ...
   */
  async generate(prompt: string, modelId: string, options?: ModelOptions): Promise<LLMResponse> {
    try {
      ...
      if (error instanceof Error) {
        ...
        if (
          ...
          throw new ApiError(`API key error: ${error.message}`, {
            ...
            examples: ['export ANTHROPIC_API_KEY=your_new_api_key'],
          });
        }
  ...
  async listModels(apiKey?: string): Promise<LLMAvailableModel[]> {
    try {
      ...
      return knownModels.map(model => ({
        ...
        pricing: {
          inputPerMillionTokens: model.id.includes('3-opus')
            ? 15
            : model.id.includes('3-sonnet')
              ? 7.5
              : model.id.includes('3-haiku')
                ? 0.25
                : model.id.includes('claude-2')
                  ? 8
                  : 1.63,
          outputPerMillionTokens: model.id.includes('3-opus')
            ? 75
            : model.id.includes('3-sonnet')
              ? 24
              : model.id.includes('3-haiku')
        ...
        contextWindow: model.id.includes('3-opus')
          ? 200000
          : model.id.includes('3-sonnet')
            ? 200000
            : model.id.includes('3-haiku')

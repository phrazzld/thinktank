...
func TestRegistryAPIWithModelsPackage(t *testing.T) {
	...
	// Test all models work with the service (14 production + 4 test)
	allModels := models.ListAllModels()
	if len(allModels) != 18 {
		t.Fatalf("Expected 18 models (14 production + 4 test), got %d", len(allModels))
	}
...
func TestProviderDistribution(t *testing.T) {
	...
	if len(openrouterModels) != 14 {
		t.Errorf("Expected 14 OpenRouter models after consolidation, got %d", len(openrouterModels))
	}
	if len(testModels) != 4 {
	...
	if total != 18 {
		t.Errorf("Expected total 18 models (14 production + 4 test), got %d", total)
	}
}

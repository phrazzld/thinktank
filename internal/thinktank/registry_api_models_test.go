...
// with the new models package for all supported models
func TestRegistryAPIWithModelsPackage(t *testing.T) {
	...
	// Test all models work with the service (17 production + 4 test)
	allModels := models.ListAllModels()
	if len(allModels) != 21 {
		t.Fatalf("Expected 21 models (17 production + 4 test), got %d", len(allModels))
	}
...
func TestRegistryAPIErrorHandling(t *testing.T) {
	...
	valid, err := service.ValidateModelParameter(ctx, "gpt-5.2", "temperature", 5.0)
	if err == nil {
...
func TestProviderDistribution(t *testing.T) {
	...
	if len(openrouterModels) != 17 {
		t.Errorf("Expected 17 OpenRouter models after consolidation, got %d", len(openrouterModels))
	}
	if len(testModels) != 4 {
	...
	if total != 21 {
		t.Errorf("Expected total 21 models (17 production + 4 test), got %d", total)
	}
}

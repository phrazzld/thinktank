...
// ErrorType represents different categories of errors that can occur when using the OpenAI API
type ErrorType int
...
// Category implements the llm.CategorizedError interface, mapping OpenAI error types
...
func FormatAPIError(err error, statusCode int) *APIError {
	...
	case ErrorTypeNetwork:
		...
		apiErr.Suggestion = "Check your internet connection and try again. If persistent, there may be connectivity issues to OpenAI's servers."

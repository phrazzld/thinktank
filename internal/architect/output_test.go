...
// Note: Temporarily commenting out complex tests to focus on the prompt stitching tests
/*
...
func TestEscapeContent(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    "if (x < 10 && y > 20) { return <result>value</result> }",
			expected: "if (x &lt; 10 && y &gt; 20) { return &lt;result&gt;value&lt;/result&gt; }",
		},
		{
			name:     "Empty string",
			input:    "",
...
func TestStitchPrompt(t *testing.T) {
	...
	fileWithTags := fileutil.FileMeta{
		Path:    "/tagged/file.go",
		Content: "func test() { if (a < b || c > d) { return <result>value</result> } }",
	}
	...
	}{
		{
			...
			checks: []func(t *testing.T, result string){
				...
				func(t *testing.T, result string) {
					...
					if !strings.Contains(result, "&lt;result&gt;value&lt;/result&gt;") {

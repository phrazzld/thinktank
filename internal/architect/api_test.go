...
// TestInitClient tests the deprecated InitClient method
func TestInitClient(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			apiEndpoint:   "",
			expectError:   true,
			errorContains: "unsupported model type",
		},
		{
			name:          "Non-Gemini Model",
			apiKey:        "test-key",
			modelName:     "gpt-4", // OpenAI model
			apiEndpoint:   "",
			expectError:   true,
			errorContains: "only supports Gemini models",
		},
		{
			name:          "Valid Gemini Model",
			apiKey:        "test-key",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			// Create API service with test client functions
			logger := &mockAPILogger{}
			apiService := &apiService{
				logger:              logger,
				newGeminiClientFunc: newGeminiClientWrapperForTest,
			...
			// Call the method being tested
			client, err := apiService.InitClient(ctx, tc.apiKey, tc.modelName, tc.apiEndpoint)
			...
			if tc.expectError {
				...
				// Check model name is correctly set
				modelName := client.GetModelName()
				if modelName != tc.modelName {
					t.Errorf("Expected model name '%s', got '%s'", tc.modelName, modelName)
				}

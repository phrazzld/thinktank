...
func TestMultiModelExecution(t *testing.T) {
	...
	// Save model names that were processed with mutex protection
	processedModelsMutex := &sync.Mutex{}
	...
	env.MockClient.GenerateContentFunc = func(ctx context.Context, prompt string) (*gemini.GenerationResult, error) {
		...
		processedModelsMutex.Lock()
		processedModels[modelName] = true
		processedModelsMutex.Unlock()
...
func TestModelFailureHandling(t *testing.T) {
	...
	// Track model processing attempts with mutex protection
...
func main() {}`)
	...
	failedModelCount := len(failedModels)
	modelsMutex.Unlock()
...
func main() {}`)
	...
	processOrder.Lock()
	startedCount := len(processOrder.started)
	completedCount := len(processOrder.completed)
	processOrder.Unlock()
...
func TestConcurrentModelFailureHandling(t *testing.T) {
	...
	// Create a synchronized struct for tracking model states
...
// TestEnhancedConcurrentModelProcessing tests that models are processed concurrently
// by checking that their processing overlaps in time using time intervals
func TestEnhancedConcurrentModelProcessing(t *testing.T) {
	// Set up the test environment
	env := NewTestEnv(t)
	defer env.Cleanup()
	...
	// Set up the barriers
	modelCount := 5 // Use more models to better test concurrency
	processingStarted.Add(modelCount)
	processingBarrier.Add(1) // Single barrier to hold all goroutines until all have started
	processingCompleted.Add(modelCount)

	// Track execution data
	processIntervals := &struct {
		sync.Mutex
		intervals []TimeInterval
	}{
		intervals: make([]TimeInterval, 0, modelCount),
	}
	...
	env.MockClient.GenerateContentFunc = func(ctx context.Context, prompt string) (*gemini.GenerationResult, error) {
		...
		// Record the interval
		processIntervals.Lock()
		processIntervals.intervals = append(processIntervals.intervals, TimeInterval{Start: startTime, End: endTime})
		processIntervals.Unlock()
...
func main() {}`)
	...
	// Create an instructions file
	instructionsFile := env.CreateTestFile(t, "instructions.md", "Test enhanced concurrent model processing")
	...
	processIntervals.Lock()
	intervals := processIntervals.intervals
	processIntervals.Unlock()

...
// TestGenerateContentParameterBoundaries tests parameter boundary validation for Gemini provider
func TestGenerateContentParameterBoundaries(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "invalid temperature way above maximum",
			prompt: "test prompt",
			parameters: map[string]interface{}{
				"temperature": 5.0,
			},
		...
		// MaxOutputTokens boundary tests
		{
			name:   "valid max_output_tokens minimum",
			prompt: "test prompt",
			parameters: map[string]interface{}{
				"max_output_tokens": 1,
			},
		...
		{
			name:   "valid max_output_tokens middle range",
			prompt: "test prompt",
			parameters: map[string]interface{}{
		...
		{
			name:   "valid max_output_tokens large value",
			prompt: "test prompt",
			parameters: map[string]interface{}{
		...
		{
			name:   "max_output_tokens as float64 (valid)",
			prompt: "test prompt",
			parameters: map[string]interface{}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Note: This will test the current implementation which may not have all validations yet
...
func TestParameterTypeConversion(t *testing.T) {
	tests := []struct {
		...
		{
			name: "max_output_tokens as different integer types",
			parameters: map[string]interface{}{
		...
		},
		{
			...
			parameters: map[string]interface{}{
				"max_output_tokens": int64(2048),
			},
			expectError: false,
		},
		{
			name: "top_k as different integer types",
			parameters: map[string]interface{}{
				"top_k": int32(40),
			},
			expectError: false,
